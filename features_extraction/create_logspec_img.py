...
train_feature_dir = 'features/'
dev_feature_dir = 'features/'
...
for nf in range(n_train_feats_files):
    filename = f'{train_feature_dir}train_logspec200_{nf}.pkl.gz'
    print(filename)
...
for nf in range(n_dev_feats_files):
    filename = f'{dev_feature_dir}dev_logspec200_{nf}.pkl.gz'
    with gzip.open(filename, 'rb') as f:
        allspec = pickle.load(f)
